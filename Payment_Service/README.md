...
└─────────────────────────────────────────────────────────────┘
         ...
         ▼                ▼                    ▼
┌─────────────────┐ ┌─────────────────┐ ┌─────────────────────┐
│  Semantic View  │ │  Semantic View  │ │ Cortex Search Svc   │
│SV_CUSTOMER_     │ │SV_CAMPAIGN_     │ │OPERATION_DOCUMENTS_ │
│  TRANSACTION    │ │  ANALYSIS       │ │        CSS          │
│(顧客・取引)     │ │(キャンペーン)   │ │(業務マニュアル等)   │
└─────────────────┘ └─────────────────┘ └─────────────────────┘
         │                │                    │
         ▼                ▼                    ▼
┌─────────────────────────────────────┐ ┌─────────────────────┐
│            DATA スキーマ             │ │    DATA スキーマ     │
│  DIM_CUSTOMER / DIM_MERCHANT        │ │ OPERATION_DOCUMENT  │
│  DIM_CAMPAIGN / FACT_TRANSACTION    │ │                     │
└─────────────────────────────────────┘ └─────────────────────┘
```
...
### 🎯 キャンペーン分析（CampaignAnalyst）
- 「キャンペーン別の取引金額と取引件数を教えて」
- 「現在実施中のキャンペーン一覧を見せて」
- 「キャンペーン種別ごとの効果を分析して」
- 「カード種別ごとのキャンペーン利用状況を教えて」
