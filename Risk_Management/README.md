...
このデモは、架空の金融機関「SFBank（SFバンク）」のリスク管理部門を想定し、以下を実現します：
...
UNION ALL SELECT 'FACT_LOAN_DETAIL', COUNT(*) FROM SF_RISK.DATA.FACT_LOAN_DETAIL
UNION ALL SELECT 'FACT_INTERNAL_RISK', COUNT(*) FROM SF_RISK.DATA.FACT_INTERNAL_RISK
UNION ALL SELECT 'FACT_REGULATORY_RISK', COUNT(*) FROM SF_RISK.DATA.FACT_REGULATORY_RISK
UNION ALL SELECT 'RISK_DOCUMENT', COUNT(*) FROM SF_RISK.DATA.RISK_DOCUMENT;
