...
CREATE WAREHOUSE IF NOT EXISTS SF_RISK_WH
    WAREHOUSE_SIZE = 'XSMALL'
    AUTO_SUSPEND = 60
    AUTO_RESUME = TRUE
    INITIALLY_SUSPENDED = TRUE
    COMMENT = 'SFBank リスク管理デモ用ウェアハウス';

USE WAREHOUSE SF_RISK_WH;

-- =============================================================================
-- 2. データベース・スキーマ作成
-- =============================================================================
CREATE DATABASE IF NOT EXISTS SF_RISK;
CREATE SCHEMA IF NOT EXISTS SF_RISK.DATA;
CREATE SCHEMA IF NOT EXISTS SF_RISK.AI;

USE DATABASE SF_RISK;
USE SCHEMA DATA;
...
CREATE OR REPLACE SEMANTIC VIEW SV_INTERNAL_RISK_ANALYSIS
  TABLES (
    LOAN AS SF_RISK.DATA.FACT_LOAN_DETAIL PRIMARY KEY (LOAN_KEY),
    INTERNAL_RISK AS SF_RISK.DATA.FACT_INTERNAL_RISK PRIMARY KEY (INTERNAL_RISK_KEY),
    COUNTERPARTY AS SF_RISK.DATA.DIM_COUNTERPARTY PRIMARY KEY (COUNTERPARTY_KEY),
    PRODUCT AS SF_RISK.DATA.DIM_PRODUCT PRIMARY KEY (PRODUCT_KEY),
    DEPARTMENT AS SF_RISK.DATA.DIM_DEPARTMENT PRIMARY KEY (DEPARTMENT_KEY),
    RATING AS SF_RISK.DATA.DIM_RATING PRIMARY KEY (RATING_KEY),
    INDUSTRY AS SF_RISK.DATA.DIM_INDUSTRY PRIMARY KEY (INDUSTRY_KEY)
  )
  RELATIONSHIPS (
...
CREATE OR REPLACE SEMANTIC VIEW SV_REGULATORY_RISK_ANALYSIS
  TABLES (
    LOAN AS SF_RISK.DATA.FACT_LOAN_DETAIL PRIMARY KEY (LOAN_KEY),
    REGULATORY_RISK AS SF_RISK.DATA.FACT_REGULATORY_RISK PRIMARY KEY (REGULATORY_RISK_KEY),
    COUNTERPARTY AS SF_RISK.DATA.DIM_COUNTERPARTY PRIMARY KEY (COUNTERPARTY_KEY),
    PRODUCT AS SF_RISK.DATA.DIM_PRODUCT PRIMARY KEY (PRODUCT_KEY),
    DEPARTMENT AS SF_RISK.DATA.DIM_DEPARTMENT PRIMARY KEY (DEPARTMENT_KEY),
    RATING AS SF_RISK.DATA.DIM_RATING PRIMARY KEY (RATING_KEY),
...
AS (
  SELECT
    ...
    CATEGORY,
    KEYWORDS
  FROM SF_RISK.DATA.RISK_DOCUMENT
);
...
CREATE OR REPLACE AGENT RISK_MANAGEMENT_AGENT
  ...
  tool_resources:
    InternalRiskAnalyst:
      semantic_view: "SF_RISK.AI.SV_INTERNAL_RISK_ANALYSIS"
    RegulatoryRiskAnalyst:
      semantic_view: "SF_RISK.AI.SV_REGULATORY_RISK_ANALYSIS"
    RiskDocumentSearch:
      name: "SF_RISK.AI.RISK_DOCUMENTS_CSS"
      max_results: 5
  $$;
...
-- データ件数確認
SELECT 'DIM_RATING' AS TABLE_NAME, COUNT(*) AS ROW_COUNT FROM SF_RISK.DATA.DIM_RATING
UNION ALL
SELECT 'DIM_INDUSTRY', COUNT(*) FROM SF_RISK.DATA.DIM_INDUSTRY
UNION ALL
SELECT 'DIM_DEPARTMENT', COUNT(*) FROM SF_RISK.DATA.DIM_DEPARTMENT
UNION ALL
SELECT 'DIM_PRODUCT', COUNT(*) FROM SF_RISK.DATA.DIM_PRODUCT
UNION ALL
SELECT 'DIM_COUNTERPARTY', COUNT(*) FROM SF_RISK.DATA.DIM_COUNTERPARTY
UNION ALL
SELECT 'FACT_LOAN_DETAIL', COUNT(*) FROM SF_RISK.DATA.FACT_LOAN_DETAIL
UNION ALL
SELECT 'FACT_INTERNAL_RISK', COUNT(*) FROM SF_RISK.DATA.FACT_INTERNAL_RISK
UNION ALL
SELECT 'FACT_REGULATORY_RISK', COUNT(*) FROM SF_RISK.DATA.FACT_REGULATORY_RISK
UNION ALL
SELECT 'RISK_DOCUMENT', COUNT(*) FROM SF_RISK.DATA.RISK_DOCUMENT;

-- オブジェクト確認
SHOW TABLES IN SCHEMA SF_RISK.DATA;
SHOW SEMANTIC VIEWS IN SCHEMA SF_RISK.AI;
SHOW CORTEX SEARCH SERVICES IN SCHEMA SF_RISK.AI;
SHOW AGENTS IN SCHEMA SF_RISK.AI;

-- サンプルデータ確認
SELECT * FROM SF_RISK.DATA.DIM_RATING ORDER BY RATING_RANK LIMIT 5;
SELECT * FROM SF_RISK.DATA.DIM_DEPARTMENT WHERE DEPARTMENT_TYPE = '営業' LIMIT 5;
SELECT * FROM SF_RISK.DATA.FACT_LOAN_DETAIL LIMIT 5;
SELECT * FROM SF_RISK.DATA.FACT_INTERNAL_RISK LIMIT 5;
SELECT * FROM SF_RISK.DATA.FACT_REGULATORY_RISK LIMIT 5;
...
-- 他ロールへの権限付与例
-- GRANT USAGE ON DATABASE SF_RISK TO ROLE <TARGET_ROLE>;
-- GRANT USAGE ON SCHEMA SF_RISK.DATA TO ROLE <TARGET_ROLE>;
-- GRANT USAGE ON SCHEMA SF_RISK.AI TO ROLE <TARGET_ROLE>;
-- GRANT SELECT ON ALL TABLES IN SCHEMA SF_RISK.DATA TO ROLE <TARGET_ROLE>;
-- GRANT USAGE ON SEMANTIC VIEW SF_RISK.AI.SV_INTERNAL_RISK_ANALYSIS TO ROLE <TARGET_ROLE>;
-- GRANT USAGE ON SEMANTIC VIEW SF_RISK.AI.SV_REGULATORY_RISK_ANALYSIS TO ROLE <TARGET_ROLE>;
-- GRANT USAGE ON CORTEX SEARCH SERVICE SF_RISK.AI.RISK_DOCUMENTS_CSS TO ROLE <TARGET_ROLE>;
-- GRANT USAGE ON CORTEX AGENT SF_RISK.AI.RISK_MANAGEMENT_AGENT TO ROLE <TARGET_ROLE>;
-- GRANT USAGE ON WAREHOUSE SF_RISK_WH TO ROLE <TARGET_ROLE>;

-- =============================================================================
-- 完了メッセージ
-- =============================================================================
SELECT '✅ SF_RISK 環境構築完了' AS STATUS,
       'データベース: SF_RISK' AS DATABASE_INFO,
