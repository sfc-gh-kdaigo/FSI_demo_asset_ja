...
CREATE DATABASE IF NOT EXISTS SNOW_INSURANCE_DB;
CREATE SCHEMA IF NOT EXISTS SNOW_INSURANCE_DB.DATA;
CREATE SCHEMA IF NOT EXISTS SNOW_INSURANCE_DB.SEMANTIC_VIEW;
CREATE SCHEMA IF NOT EXISTS SNOW_INSURANCE_DB.CORTEX_SEARCH;
CREATE SCHEMA IF NOT EXISTS SNOW_INSURANCE_DB.AGENTS;
CREATE WAREHOUSE IF NOT EXISTS COMPUTE_WH 
    WITH WAREHOUSE_SIZE = 'XSMALL' 
    AUTO_SUSPEND = 60 
    AUTO_RESUME = TRUE;

USE DATABASE SNOW_INSURANCE_DB;
USE SCHEMA DATA;
...
-- ============================================================
CREATE OR REPLACE CORTEX SEARCH SERVICE SNOW_INSURANCE_DB.CORTEX_SEARCH.INSURANCE_DOCUMENTS_SEARCH_SERVICE
    ON CONTENT
    ...
    AS (
        SELECT 
            ...
            UPDATED_AT,
            VERSION
        FROM SNOW_INSURANCE_DB.DATA.INSURANCE_DOCUMENTS
    );
...
-- ------------------------------------------------------------
CREATE OR REPLACE SEMANTIC VIEW SNOW_INSURANCE_DB.SEMANTIC_VIEW.CUSTOMER_CONTRACT_ANALYSIS_SV
    TABLES (
        SNOW_INSURANCE_DB.DATA.CUSTOMERS
            PRIMARY KEY (CUSTOMER_ID)
            COMMENT = '顧客の基本情報を管理するマスタテーブル。顧客ID、氏名、生年月日、年齢、性別、婚姻状況、職業、年収、住所、顧客セグメント、リスクレベル等を含む。',
        SNOW_INSURANCE_DB.DATA.FAMILY_MEMBERS
            PRIMARY KEY (FAMILY_ID)
            COMMENT = '顧客の家族構成情報を管理するテーブル。家族ID、顧客ID、続柄、氏名、生年月日、年齢、性別、職業、扶養有無を含む。',
        SNOW_INSURANCE_DB.DATA.INSURANCE_PRODUCTS
            PRIMARY KEY (PRODUCT_ID)
            COMMENT = '保険商品のマスタテーブル。商品ID、商品名、カテゴリ、タイプ、契約可能年齢範囲、補償額範囲、保険料計算方法、商品説明を含む。',
        SNOW_INSURANCE_DB.DATA.INSURANCE_CONTRACTS
...
-- ------------------------------------------------------------
CREATE OR REPLACE SEMANTIC VIEW SNOW_INSURANCE_DB.SEMANTIC_VIEW.CLAIM_ANALYSIS_SV
    TABLES (
        SNOW_INSURANCE_DB.DATA.CLAIM_HISTORY
...
-- ============================================================
CREATE OR REPLACE AGENT SNOW_INSURANCE_DB.AGENTS.LIFE_INSURANCE_INTELLIGENCE
...
tool_resources:
  Customer_Contract_Analysis:
    semantic_view: "SNOW_INSURANCE_DB.SEMANTIC_VIEW.CUSTOMER_CONTRACT_ANALYSIS_SV"
  Claim_Analysis:
    semantic_view: "SNOW_INSURANCE_DB.SEMANTIC_VIEW.CLAIM_ANALYSIS_SV"
  Insurance_Document_Search:
    name: "SNOW_INSURANCE_DB.CORTEX_SEARCH.INSURANCE_DOCUMENTS_SEARCH_SERVICE"
    max_results: "5"
$$;
